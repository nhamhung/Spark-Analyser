...
/**
  * Optimize to reduce Spark application runtime.
  */
class RuntimeSavingOptimizer extends Optimizer {
  ...
  override def optimize(app: AppData): Optimization = {
    ...
    val numPartitions = recs.getMax(Recommendation.maxNumPartitions).get
    ...
    println("CURRENT APP TOTAL MEM: ", currAppTotalMem)
    println("NUM PARTITION: ", numPartitions)
    println("NUM PARTITIONS PER CORE: ", numPartitionsPerCore)
    println("NUM CORES PER EXECUTOR: ", numCoresPerExecutor)
    println("NUM EXECUTORS: ", numExecutors)
    println("MEM PER EXECUTOR: ", memPerExecutor)
    ...
    val storageFraction = Optimizer.DEFAULT_STORAGE_FRACTION
